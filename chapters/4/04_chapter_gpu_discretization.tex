...
using our incremental modeling enviroment. To make a fair comparison the same algorithm implemented on the GPU using
...
When using stacks register spilling mentioned previously will degrade the performance due to the higher cost of accessing shared memory. 
Conditional push and pops in stack-based method also stalls the performance of the kernels.  

Faster field evaluations using the stackless algorithm also boosts the performance of our root-finding method and the overall
...
\begin{figure}[H]
  ...
  stage due to the gradient-based Newthon-Raphson root finding method employed and the evaluation of colors and normals which require additional traversals.
  }
}
\end{figure}
